...
SELECT district.name AS district_name, region.name AS region_name,
  SUM(CASE
        WHEN b.beneficiary_type = 'HH' THEN b.beneficiaries * 6
  ...
  ROUND(
    SUM(CASE
          WHEN b.beneficiary_type = 'HH' THEN b.beneficiaries * 6
          ELSE b.beneficiaries
        END) * 1.0 / NULLIF(SUM(v.total_population), 0), 4
  ) AS beneficiaries_per_population_ratio
FROM beneficiary_partner_data b
JOIN village_locations v ON b.village = v.village
JOIN jurisdiction_hierarchy village ON b.village = village.name AND village.level = 'Village'
JOIN jurisdiction_hierarchy district ON village.parent_name = district.name AND district.level = 'District'
JOIN jurisdiction_hierarchy region ON district.parent_name = region.name AND region.level = 'Region'
GROUP BY district.name, region.name;

CREATE VIEW partner_summary AS
SELECT beneficiary_partner_data.partner AS partner_name,
  COUNT(DISTINCT beneficiary_partner_data.village) AS number_of_villages_reached,
  COUNT(DISTINCT district.name) AS number_of_districts_reached
FROM beneficiary_partner_data
JOIN jurisdiction_hierarchy AS village ON beneficiary_partner_data.village = village.name AND village.level = 'Village'
JOIN jurisdiction_hierarchy AS district ON village.parent_name = district.name AND district.level = 'District'
GROUP BY beneficiary_partner_data.partner;
